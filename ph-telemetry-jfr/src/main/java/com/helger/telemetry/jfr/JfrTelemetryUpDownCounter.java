...
/**
 * {@link ITelemetryUpDownCounter} that sums up in process and is sampled into
 * {@code com.helger.telemetry.UpDownCounter} JFR events by the periodic hook of
 * {@link JfrTelemetryMeterSPI}.
 *
 * @author Philip Helger
 */
@ThreadSafe
final class JfrTelemetryUpDownCounter implements ITelemetryUpDownCounter
{
  private final String m_sName;
  private final String m_sUnit;
  private final JfrValueAccumulator m_aAccumulator;

  JfrTelemetryUpDownCounter (@NonNull final String sName, @Nullable final String sUnit)
  {
    m_sName = sName;
    m_sUnit = sUnit;
  ...
  void commitSnapshot ()
  {
    m_aAccumulator.forEachValue ((sAttributes, nValue) -> {
      final JfrUpDownCounterEvent aEvent = new JfrUpDownCounterEvent ();
